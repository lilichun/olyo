    ...
    implements OperationSetBasicInstantMessaging
{
    ...
    private DHTAccessService dhtAccessor;
        ...
        throws OperationFailedException
    {
        ...
        try
        {
            ...
            toAddress = resolveInDHT(toAddress);

            logger.info("After resolve toAddress in createMessage()" + ((SipURI) toAddress.getURI()).getHost());
            
            destinationInetAddress = InetAddress.getByName(
                ( (SipURI) toAddress.getURI()).getHost());
            
            logger.info("destinationInetAddress = " + destinationInetAddress);
        }
    ...
    /**
     * Resolve a SIP Address using DHT GET 
     * @author Wang Yao
     * 
     * @param toAddress: the address to be resolved
     * @return IP if the input address is successfuly resolved
     *         the_input_address otherwise
     */
    private Address resolveInDHT(Address toAddress) {
        ...
        for(Iterator it = resultSet.iterator(); it.hasNext();){
        	...
        	if(null != addrStrFromDHT){
        		try {
					return parseAddressStr(addrStrFromDHT);
        ...
        implements SipListener
    {
        ...
        {
		logger.info("Enter processRequest() in OperationSetBasicInstantMessagingSipImpl");
		synchronized (messageProcessors)
            {
		logger.info("ENter Synchronized block");
