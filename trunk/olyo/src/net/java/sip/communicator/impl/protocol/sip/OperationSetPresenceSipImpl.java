...
implements OperationSetPersistentPresence, SipListener
{
	 ...
	 *
	 *it is a verdict param 
	 */


	private boolean enableP2PSIP=true;


	/**
	 *this is the p2psipuser's ip address.
	 */

	private String p2psiptodomain=null;

	private String p2psipfromdomain=null;
...
//	@author Dong
	...
	// private ContactSipImpl contactSipImpl=null;
			...
			int subscriptionExpiration)
	{
		 ...
		 **/
		enableP2PSIP=provider.isP2PSIP();
...
//			added by dongfengyu
	...
	throws OperationFailedException
	{
		...
		try
		{
			...
			logger.info( "Request createPublish"+req);
	 ...
	 *@return a String
	 *
	 *the function below is used to get the IP from the value which got from DHT.
	 *
	 */
	String getSubString(String original, char begin, char end )
	{
		int indexBegain=original.indexOf(begin)+1;

		int indexEnd=original.lastIndexOf(end);
	...
	/*
        ...
        String getSubString(String original)
	{
		int indexBegain=original.indexOf('@')+1;
	 ...
	 */
	/*String getUsername(String original) 
		{
		int indexBegain=original.indexOf(':')+1;
	 ...
	 *
	 *Added the P2PSIP part by Dong Fengyu
	 */
	...
	throws OperationFailedException
	{
		...
		Address p2psiptoAddress=null;
		Address p2psipfromAddress=null;


		try{	
			toAddress = parseAddressStr(contact.getAddress());

			logger.info("contact.getAddress() ="+contact.getAddress());
		...
		if(false==enableP2PSIP){
			try
			{     
				destinationInetAddress = InetAddress.getByName(
						((SipURI) toAddress.getURI()).getHost());

				logger.info("sip-destinationInetAddress = "+destinationInetAddress.toString());
			...
			Set resultSet = null;
			try{
				resultSet = dhtAccessor.get(toAddress.toString().trim());
			...
			if (0 == resultSet.size()){
				logger.info("No result found in DHT. key = " + toAddress.toString().trim());
				throw new OperationFailedException(
						"The target contact is not online.", 
						OperationFailedException.NOT_FOUND);
			}

			String addrStrFromDHT = null;	

			for(Iterator it = resultSet.iterator(); it.hasNext();){
				addrStrFromDHT = new String((byte[])it.next());


				logger.info("addrStrFromDHT="+addrStrFromDHT );

				p2psiptodomain=getSubString(addrStrFromDHT,'@',':');

				logger.info("p2psiptodomain"+p2psiptodomain);

				try
				{
					p2psipfromdomain=InetAddress.getLocalHost().getHostAddress();

					logger.info("p2psipfromdomain"+p2psipfromdomain);

				}
				catch(UnknownHostException e )
				{
					logger.error("could not find LocalHost computer's address");

				}


				try {
					p2psiptoAddress=parseAddressStr(contact.getAddress(),true);
					destinationInetAddress = InetAddress.getByName(
							((SipURI) p2psiptoAddress.getURI()).getHost());
					logger.info("p2psip-destinationInetAddress="+InetAddress.getByName(
							((SipURI) p2psiptoAddress.getURI()).getHost()));
				} catch (ParseException e) {
					logger.error("ParseException occured when parseAddressStr(addrStrFromDHT).\n" +
							"addrStrFromDHT = " + addrStrFromDHT,
							e);
				}
				catch (UnknownHostException ex)
				{
		...
		try
		{
			...
			if(false==enableP2PSIP){
				...
				logger.info("sipuser-fromHeader ="+fromHeader); 	
			}
			else{

				//String p2psipuser=getUsername(this.parentProvider.getOurSipAddress().toString());
				String p2psipuser=getSubString(this.parentProvider.
						getOurSipAddress().toString(),':','@');

				logger.info("p2psipuser ="+p2psipuser);

				try{
					p2psipfromAddress=parseAddressStr(p2psipuser,false);

				}
				catch (ParseException e) {
					logger.error("ParseException occured when p2psipfromAddress.\n" +
							e);
				}

				fromHeader = this.parentProvider.getHeaderFactory()
				.createFromHeader(p2psipfromAddress , localTag);

				logger.info("p2psipuser-fromHeader ="+fromHeader); 	
			}	
			...
			else
			{
				...
				toHeader = this.parentProvider.getHeaderFactory()
				.createToHeader(p2psiptoAddress, null);
			}
		}
		catch (ParseException ex)
		{
		...
		logger.info("Request createSubscription---req ="+req);	
	 ...
	 *
	 * @this function is used for sipuser!
	 *     
	 */
	 ...
	 *Added by Dong Fengyu
	 *
	 *@this is a reload function
	 *
	 * the function below is used to parse the username into sipuri
	 * like this structure:sip:username@userIP.
	 *
	 *this function is used for p2psipuser!
	 *  
	 */

	private Address parseAddressStr(String uriStr,boolean toAddress)
	throws ParseException
	{
		String res = uriStr.trim();

		//it is not handle default domain name .
		//we can get the p2psipuser domain from the Param definition above,
		// this Param is a string,but actually,it is a IP address of the host which we want to connect.
		if (res.indexOf('@') == -1)
		{
			if(true==toAddress){
				res=res+'@'+p2psiptodomain.trim();
			}
			else
			{
				res=res+'@'+p2psipfromdomain.trim();
			}
		}
...
Added by Dong Fengyu
	...
	public ContactSipImpl createVolatileContact(String contactAddress)
	{
		...
		if(true==enableP2PSIP)
		{

			contactAddress=getUsername(contactAddress);
		}
...
//			added by Dong Fengyu
	...
	public byte[] getPidfPresenceStatus(ContactSipImpl contact)
	{
		...
		// logger.info("in getPidfPresenceStatus() contact ="+contact);
		//contactSipImpl

//		logger.info("in getPidfPresenceStatus() contact ="+contactSipImpl);
		...
		try {
			...
			/*
               ...
               contactUri = this.parseAddressStr(contactSipImpl.getAddress())
                 .getURI().toString();  
			 */

			logger.info("in getPidfPresenceStatus() contacturi ="+contactUri );
	...
	public void forcePollContact(ContactSipImpl contact) {
		...
		{
			if (OperationFailedException.NOT_FOUND 
					== ex.getErrorCode()){
				logger.info(
						"The target user for subscribtion is not online.", ex);
				return;
			}
			logger.error(
