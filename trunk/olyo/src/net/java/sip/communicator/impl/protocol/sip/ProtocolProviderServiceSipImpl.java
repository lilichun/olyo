...
implements ProtocolProviderService, SipListener
{
	...
	/**
	 * the two parameters below are used in many locations,they show that 
	 * if the user uses the P2PSIP protocol.
	...
	public boolean isRegistered()
	{
		...
		logger.debug("If this line is printed out, it means null!=this.sipRegistrarConnection.");
		logger.debug("The status for sipRegistrarConnection is: " + sipRegistrarConnection.getRegistrationState());
		return sipRegistrarConnection.getRegistrationState()
	...
	throws OperationFailedException, IllegalArgumentException
	{
		synchronized (initializationLock)
		{
			...
			//Commented by WangYao, we do not need SIP OUTBOUND_PROXY in P2PSIP
			//init the proxy
			...
			String enableP2PSIPObj=(String)accountID.
			getAccountProperties().get(
					ProtocolProviderFactory.IS_P2PSIP_ENABLED);
			enableP2PSIP =true;
			if(enableP2PSIPObj!=null){
				enableP2PSIP=Boolean.valueOf(enableP2PSIPObj).booleanValue();
			}


			String enablePassCheckObj=(String)accountID.
			getAccountProperties().get(
					ProtocolProviderFactory.IS_PASSCHECK_ENABLED);
			enablePassCheck=false;
			if(enablePassCheckObj!=null){
				enablePassCheck=Boolean.valueOf(enablePassCheckObj).booleanValue();
			}
			...
			/*	Commented by WangYao
			 *     Added the if-else sentence by Dong Fengyu
			 */
			 ...
			 */
				...
				if(sipUriHost == null){
					...
					sipUriHost = registrarAddressStr;
				}
			}
			...
			transactionTerminatedEvent)
	{
		...
		logger.info("transactionTerminatedEvent.isServerTransaction()="
				+transactionTerminatedEvent.isServerTransaction());

		if(transactionTerminatedEvent.isServerTransaction())
		{
			transaction = transactionTerminatedEvent.getServerTransaction();
			logger.info("transactionTerminatedEvent.isServerTransaction()==ture="
					+transactionTerminatedEvent.isServerTransaction())	;
		}
		...
		else
		{
			...
			logger.info("transactionTerminatedEvent.isServerTransaction()==false=");
		}
		...
		logger.debug("transaction.getRequest() ="+request)	;
...
//				this.jainSipStack.stop();
	...
	throws IllegalArgumentException
	{
		...
		/**
		 * Commented by WangYao
		 * In P2PSIP, it is unnecessary to give a valid IP addr for SIP registrar.
