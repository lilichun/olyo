...
public class NetworkUtils
{
    ...
    /**
     * Added by WangYao
     * This method tells whether a String is a valid IPv4 addr 
     * in the form of "xxx.xxx.xxx.xxx", where "xxx" ranges from 0 to 255
     * @param ipStr String to be tested
     * @return true if the String is a valid IPv4 addr.
     */
    public static boolean isDecimalDotFormatIPv4Addr(String ipStr){
    	if ("0.0.0.0".length() > ipStr.length()
    			|| "255.255.255.255".length() < ipStr.length()){
    		return false;
    	}
    	
    	List<Integer> dotIndexList = new ArrayList<Integer>();
    	for(int i=0; i<ipStr.length(); i++){
    		if ('.' == ipStr.charAt(i)){
    			dotIndexList.add(i);
    		}
    	}
    	if (3 != dotIndexList.size()){
    		return false;
    	}
    	
    	try{
    		int item = Integer.parseInt(ipStr.substring(0, dotIndexList.get(0)));
    		if (0 > item || 255 < item){
    			return false;
    		}
    		
			item = Integer.parseInt(ipStr.substring(dotIndexList.get(0) + 1, dotIndexList.get(1)));			
    		if (0 > item || 255 < item){
    			return false;
    		}
    		
    		item = Integer.parseInt(ipStr.substring(dotIndexList.get(1) + 1, dotIndexList.get(2)));
    		if (0 > item || 255 < item){
    			return false;
    		}
    		
    		item = Integer.parseInt(ipStr.substring(dotIndexList.get(2) + 1));
    		if (0 > item || 255 < item){
    			return false;
    		}
    ...
    /**
     * Added by WangYao
     * This method tells whether a int is a valid port number. 
     * @param port (int) the port number to be tested 
     * @return true if the int is a valid port number.
     */
